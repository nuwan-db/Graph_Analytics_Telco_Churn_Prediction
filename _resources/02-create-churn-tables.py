...
dbutils.widgets.text("root_folder", "lakehouse-telco-churn", "Demo root folder")
...
# MAGIC %run ./00-global-setup-bundle $reset_all_data=$reset_all_data $db_prefix=telco $catalog=$catalog $db=$db
...
if not spark._jsparkSession.catalog().tableExists(f"`{catalog}`.`{database}`.`churn_orders_bronze`") or \
  ...
  qo = ingest_folder('/demos/telco/churn/orders', 'json', 'churn_orders_bronze')
  qe = ingest_folder('/demos/telco/churn/events', 'csv', 'churn_app_events')
  qu = ingest_folder('/demos/telco/churn/users', 'json',  'churn_users_bronze')
